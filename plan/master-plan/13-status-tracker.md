...
| MP-09 Optional product enhancements | todo | not started | not started | scope tightened to optional local-first polish only: Markdown/YAML report export, minimal recent-run history, and enhanced single `/metrics` surface mirrored in MCP; no optional persistence layer, no duplicate status endpoints, no job system |
